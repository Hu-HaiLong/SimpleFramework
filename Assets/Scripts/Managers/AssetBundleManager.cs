...
public class AssetBundleManager : SingleTonMono<AssetBundleManager>
{
...
#elif UNITY_STANDALONE_WIN || UNITY_EDITOR
        "file://" + "E:/StreamingAssets"+ "/Windows/"; //"file://" + Application.streamingAssetsPath + "/Windows/";
#else
        string.Empty;
#endif
    }

    public void LoadServerAssetBundle()
    {
        StartCoroutine(LoadFromAB(PathURL + "textures/bg-02.assetbundle"));
        StartCoroutine(LoadAssetBundle(PathURL+ "prefabs/main.assetbundle",1, "Main"));
    }

    public void LoadLocalAssetBundle()
    {
        url = Application.dataPath + "/StreamingAssets/Windows/";
    ...
    #region server load
    //加载AB包（不提取资源）
    private IEnumerator LoadFromAB(string ABURL)
    {
    ...
    private IEnumerator LoadAssetBundle(string url, int version, string bundleName)
    {
        using (WWW www = new WWW(url))
        {
            ...
            else
            {
                ...
                GameObject cube = Instantiate(bundle.LoadAsset(bundleName), parent) as GameObject;
                bundle.Unload(false);
                bundle = null;
            }
        }
    ...
    {
        var bundle = AssetBundle.LoadFromFile(System.IO.Path.Combine(url, "Windows"));
    ...
    private void LoadBundle(string bundleName = "prefabs/main.assetbundle")
    {
        ...
        {
            AssetBundle.LoadFromFile(System.IO.Path.Combine(url, dependence[i]));
        }

        var bundle = AssetBundle.LoadFromFile(System.IO.Path.Combine(url, bundleName));
        var asset = bundle.LoadAsset("Main");
    ...
    void Download2Local()
    {
        string savePath = Application.dataPath + "/StreamingAssets/Windows";
        string _pathUrl = "http://www.yishimu.com/UnityFiles/ProjectDemo/StreamingAssets/Windows"; //"file://" + "E:/StreamingAssets" + "/Windows"

        try
        {
            DownLoadAssets2LocalWithDependencies(_pathUrl, "Windows", "prefabs/main.assetbundle", savePath, () =>
            {
                //GameObject obj = GetLoadAssetFromLocalFile("Windows", "main", "Main", savePath);
    ...
    IEnumerator DownLoadAssetsWithDependencies2Local(string AssetsHost, string RootAssetsName, string AssetName, string saveLocalPath, dlg_OnAssetBundleDownLoadOver OnDownloadOver = null)
    {
        ...
        if (RootAssetsName != "")    //总依赖项为空的时候去加载总依赖项  
        {
            ...
            if (ServerManifestWWW.error != null)
            {
                Debug.LogError(ServerManifestWWW.error);
            }
            else
            {
        ...
        foreach (var item in dicDownloadInfos)
        {
            ...
            while (!wwwAsset.isDone)
            {
                ...
                Debug.Log(string.Format("下载 {0} : {1:P1}", item.Key, wwwAsset.progress));
